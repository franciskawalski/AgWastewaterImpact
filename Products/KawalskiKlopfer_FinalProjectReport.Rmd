...
Several methods were used to estimate the effects of wastewater reuse regulations on nitrogen concentrations in bodies of water; all of these were conducted at the sub-state, individual monitoring station level. The dependent variable of each model was log-transformed. Our first method was a simple naive OLS regression. While lacking in complexity, this model with its numerous fixed effects manages to capture a substantial amount of variance in water quality and identifies effects of legislation with some degree of detail due to the individual water quality monitor level outcomes. It was estimated with the equation:
...
Our second method was a more in-depth linear regression that analyzed changes in nitrogen concentration as a result of different specifications for nitrogen limits imposed by each state. We condcuted this analysis to evaluate whether or not intensity of treatment has any effect on nitrogen concentrations. This model was estimated with the equation:

$Nitrogen_{siymr} = \beta1AmountSpecified{sy} + \beta2NotSpecified_{sy} + \beta_3State_s + \beta4Station_i + \beta_5Year_y + \beta_6Month_m + \beta7Region_r$
...
Our final model was a Gardner difference-in-differences model. Recent literature has suggested that staggered treatment difference in differences models can have biased estimates. As a result, we utilized this Gardner (2020) two-stage model in an attempt to account for any potential bias.
...
regTypeImpactNitrogen = feols(lnMeasureValue ~  regAmountSpecified + regAmountNotSpecified | 
                                State + Year + Station + Month + Region^Year, 
                              data = data, cluster = "Station")
...
Our naive regression indicated that the presence of regulations on wastewater reuse is correlated with a reduced nitrogen concentrations being reduced by 2.4%. With a standard error of 0.014, this result is statistically significant at the 10% level. This reduction indicates that regulations on the quality of reused wastewater could be effective at reducing nitrogen pollution. If we were to lend a causal inference to this result, the primary difference would be the assumption that regulations directly cause this reduction in nitrogen concentrations. This assumption is unlikely to hold, as a number of other factors not present in the model could also be affecting this data generating process. One such example is the possibility of other effects that occur simultaneously with treatment. Legislation rarely happens in isolated instances, and it is possible that states set out with a goal to reduce pollution and simultaneously enact several policies to that end. For example, if a state were to enact a policy limiting use of fertilizer at the same time as they regulated wastewater reuse, our estimate would overstate the effects of wastewater reuse regulations.

Other problems could arise from intensity of treatment, as not all states passed identical regulations. Looking at distinct types descriptively, we find that states regulating–but not specifying–allowable amounts of nitrogen in reused wastewater are correlated with reductions of nitrogen concentrations of 3.5%. With a standard error of 0.012, this result is significant at the 1% level. States that did specify allowable amounts of nitrogen in reused wastewater were correlated with an increase in nitrogen concentrations of 3.3%, though this result was not statistically significant. This result trends in the opposite direction from what was expected. Combined with its lack of significance, it is evident that other factors influence this data generating process that are not captured by our model. Therefore, we are unable to determine causality from this estimation.

While bias from outside sources is likely, it is also very possible that the estimates used above have internal bias from the models used. To mitigate this, we used a Gardner differences in differences model to estimate the effects. The Gardner model finds a statistically significant effect similar in size to our other models. However, the results are quite noisy, with a substantial, high-variance increase in the first three years post treatment, subsequent years of decreased nitrogen concentrations with small variances, and then more high-variance increases in nitrogen concentrations after 10 years. Here again, we hesitate to assign causality, despite the statistically significant result. This is because the noise within our results indicate that other factors not included within our model may be influencing the results in ways we cannot predict.

![Figure 1](images/GardnerPlot.png)

## Conclusion

Overall, this analysis finds evidence that regulating wastewater reuse could bring about a significant decrease in nitrogen pollution, but is unable to effectively ascribe causality. Future research would be much more effective when controlling for other factors that may reduce nutrient pollution–such as fertilizer regulations and other agricultural practices. Additionally, this analysis fails to capture the dynamic nature of water. Accounting for water movement using watershed-based spatial analysis could better identify the true nature of pollution effects from reused wastewater. These facators together are especially important, as agricultural activity upstream from each monitoring station may not be constant–another potential source of noise in our analysis. Despite these challenges, it is important that research continues to evaluate the reuse of wastewater in agricultural irrigation, both as a potential solution to water scarcity and as a potential source of pollution. Understanding those trade-offs may be key to a future of sustainable agriculture.
