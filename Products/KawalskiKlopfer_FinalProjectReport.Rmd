...
Where *Nitrogen* is percent change in nitrogen concentration, *Regulation* is whether or not a water quality regulation was in place in that state at that time, *Station* is the individual water quality monitoring station, *Year* is the year each observation was recorded, *Month* was the month each observation was recorded, and *Region* is the USDA Farm Production Region.
...
\$ Nitrogen\_{xzsiymr} = \beta\*1AmountSpecified\*{xsy} + \beta2NotSpecified\_{zsy} + \beta\_3State_s + \beta4Station_i + beta_5Year_y + \beta\_6Month_m + \beta7Region_r \$

Where x is the regulated amount of nitrogen specified by a state regulation, z is whether or not a state specified an amount in their regulation,
...
regImpactNitrogen = feols(lnMeasureValue ~  currentRegTreated | state  + MonitoringLocationIdentifier + month + USDA.Farm.Production.Region^year, cluster =  "MonitoringLocationIdentifier", data = data)

modelsummary(regImpactNitrogen, stars = T, output = "output.html")
